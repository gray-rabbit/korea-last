<script lang="ts">
	...
	const datas = ['가', '나', '다', '라', '마', 'ㄱ', '각', '낙', '닥', '락', '막'];
	const datas2 = ['버', '서', '저', '터', '퍼', 'ㄱ', '벅', '석', '적', '턱', '퍽'];
...
<div class="pt-4 px-4">
	...
	<div class="border-2 rounded-lg p-2 border-gray-400">
		<p class="text-2xl">
			ㄱ(기<Textimpact classstring="text-blue-500">역</Textimpact>)받침에서 <Textimpact
				classstring="text-blue-500">/윽/</Textimpact
			> 소리가 나요.
		</p>
	</div>
	<br />
	<p class="text-3xl">'ㄱ'의 소리를 눌러서 확인하세요.</p>
	<div class="border-2 rounded-lg flex w-full justify-center py-5">
		<div class="flex justify-center items-center min-w-[100px] bg-blue-100 rounded-xl">
			<button
				...
				on:click={play_element_value_by_slow}>ㄱ</button
			>
		</div>
		<div class="flex flex-col justify-center items-center min-w-[100px]">
			<button
				...
				data-text="기"
				on:click={play_element_value_by_slow}>기</button
			>
			<button
